...
| 사용자 피드백 | 완료 v1 | 정확성/오탐/누락과 수정 금액 저장, 전체 통계·필터 관리 화면 |
...
분석 기록과 사용자 피드백 관리 기능 추가 후 로컬 검증 결과는 `80 passed, 1 skipped`였다. `skipped` 항목은 기본 실행에서 제외한 OCR 전용 테스트다.
...
| P1 | 사용자 피드백 검수·내보내기 | 검수 상태를 추가하고 승인된 학습용 데이터만 내보내기 |
