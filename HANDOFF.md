...
| 사용자 피드백 | 완료 v1 | 저장·통계·검수, 승인 품질 규칙, 최소 표본 충족 시 CSV·JSON 내보내기 |
...
분석 기록과 사용자 피드백 품질 검수·내보내기 기능 추가 후 로컬 검증 결과는 `85 passed, 1 skipped`였다. `skipped` 항목은 기본 실행에서 제외한 OCR 전용 테스트다.
...
| P1 | 실제 피드백 표본 확대 | 적합 승인 20항목·서로 다른 분석 5건 이상 확보 |
